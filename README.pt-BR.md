...
> **Status**: `@christopher_dondici/mcp-gen` 2.3.3 é a última versão publicada no npm. Veja as [notas de release](RELEASE_NOTES.md).
...
- `dist` limpa antes de cada build, sem artefatos stale em builds ou tarballs.
- Verificação local: 18 suites / 234 testes Jest, smoke de exit codes 3/3, smokes TypeScript/Python/Go, `npm audit --omit=dev` limpo.

## Novidades da 2.3.2

(Patch sem breaking changes.)

- `js-yaml` e `openapi-types` agora são dependências diretas; a leitura de YAML no security-lint usa import tipado.
- Validação de URL do registry com testes reais: URLs não-HTTPS e chaves desconhecidas/removidas falham sem chamar `fetch`.
- Globs com `/**` no final também casam o path base (`/pets/**` inclui `/pets`).
- Marcadores incrementais: padrão de stub default para Go e round-trip dos guards `<generated:handlers:name>` junto dos marcadores legados `@@mcp-gen`.
- Novo smoke de compilação Go (`npm run test:generated:go`): o scaffold Go do Petstore roda `go mod tidy` + `go build`, também na CI com Go 1.22.
- Verificação local: 17 suites / 232 testes Jest, smoke de exit codes 3/3, smokes TypeScript e Go, `npm audit --omit=dev` limpo.
...
Instale a última versão com `npm install -g @christopher_dondici/mcp-gen@2.3.3`. Para trabalhar pelo código-fonte, use o build em [Início rápido](#início-rápido).
...
| v2.3.3 | Publicada no npm (atual) | Erro explícito de `--env-file` inexistente, next steps de Go no `init`, placeholders trocados, `dist` limpa antes do build |
| v2.3.2 | Publicada no npm | Deps diretas js-yaml/openapi-types, testes reais do registry, `/**` final casa a base, stub Go + round-trip dos guards, smoke Go na CI |
| v2.3.1 | Publicada no npm | Lockfile no scaffold TypeScript, precedência de force sobre incremental, exit codes confiáveis no `watch --once`, lint reproduzível |
...
| Distribuição | Verificada para a 2.3.3 | Instalação pelo registry funciona com `npm install -g @christopher_dondici/mcp-gen`; publicação via pip não comprovada. Python é um target de geração, não uma forma de instalar esta CLI via pip |
